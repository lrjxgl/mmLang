	...
	<view>
		<table class="tbs">
			...
			<tr v-for="(item,index) in list" :key="index">
				<td><checkbox class="ids" name="ids[]" value="{$c.id}" /> {{item.id}}</td>
				<td>{{item.title}}</td>
				<td>

					<img v-if="item.imgurl!=''" :src="item.imgurl+'.100x100.jpg'" width="50" />
				...
				<td>
					<div @click="toggleRecommend(item)" :class="item.is_recommend==1?'yes':'no'"></div>
				</td>
